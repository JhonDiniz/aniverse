...
#
# CANVAS_W foi alargado de 210 pra 380 especificamente por causa de poses tipo
# "dash" (corrida/investida) -- são bem mais LARGAS que altas, e um canvas
# retrato force elas a encolher pra caber (o personagem "diminui" por um
# instante durante a animação). Um canvas mais largo deixa a pose de dash
# em tamanho real, e nas poses paradas (base/block) só sobra mais margem
# transparente nas laterais -- inofensivo, porque o jogo sempre exibe pela
# ALTURA (width:auto no CSS).
CANVAS_W, CANVAS_H = 380, 245
BODY_H             = 220          # altura do corpo (topo do cabelo -> sola)
ANCHOR_X           = 190          # centro horizontal do corpo (CANVAS_W/2)
...
def make_sheet(slug, states, out_root, keep_individual=False):
    """Combina os PNGs de estado já processados (base.png, dash.png, ...) num
    único sheet.png lado a lado, na ordem dada -- e por padrão APAGA os
    arquivos individuais depois, porque o objetivo é sobrar 1 arquivo só no
    assets/sprites/<slug>/, não N.

    Os PNGs de entrada precisam ter sido gerados pelo processamento normal
    (--slug/--state) primeiro, pra já estarem no canvas comum -- combinar
    imagens de tamanhos diferentes lado a lado desalinharia o chão entre elas.
    """
    d = os.path.join(out_root, slug)
    paths = [os.path.join(d, st + ".png") for st in states]
    faltando = [p for p in paths if not os.path.exists(p)]
    if faltando:
        sys.exit(f"Faltam arquivos: {faltando}\n"
                  f"Processe cada estado primeiro com --slug {slug} --state <estado>.")

    frames = [Image.open(p).convert("RGBA") for p in paths]
    sizes = {f.size for f in frames}
    if len(sizes) > 1:
        sys.exit(f"Os estados não têm o mesmo tamanho de canvas ({sizes}) -- "
                  f"reprocesse todos com o prepare_sprite.py atual antes de juntar.")
    w, h = frames[0].size
    ...
    print(f"{out}  ({os.path.getsize(out)//1024} KB, {len(frames)} pose(s): {', '.join(states)})")

    if not keep_individual:
        for p in paths:
            os.remove(p)
        print("Arquivos individuais removidos -- só sheet.png ficou na pasta.")
    print(f"\nEm characters.py: sprite=dict(slug=\"{slug}\", states={states!r})")
...
def main():
    ...
    ap.add_argument("--sheet", metavar="SLUG",
                    help="junta os estados já processados num sheet.png só e apaga "
                         "os arquivos individuais (rode --check ANTES disso, não depois: "
                         "depois do --sheet os arquivos individuais não existem mais)")
    ap.add_argument("--states", nargs="+",
                    help="lista de estados a juntar, na ordem -- usado com --sheet. "
                         "Ex: --sheet naruto --states base dash block")
    ap.add_argument("--keep-individual", action="store_true",
                    help="com --sheet, mantém os arquivos por estado além de criar o sheet.png")
    args = ap.parse_args()

    if args.sheet:
        if not args.states:
            ap.error("--sheet precisa de --states (ex: --states base dash block)")
        make_sheet(args.sheet, args.states, args.out, args.keep_individual)
        return
    if args.check:
